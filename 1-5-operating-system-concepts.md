...
经常会发生这样的情况，某项技术的发展让一些创意显得过时，并且让他们很快就消失在人们的视野中。然而，另一项技术又可能让他们起死回生。当技术革新影响的是各个组件之间组合性能时候，这一现象则更加明显。比如，当CPU比内存速度更快时，缓存就因其能加速内存读取而变得十分重要。如果内存技术发展到让他比CPU还快，缓存就消失了。但是如果CPU技术再发展到再次快于内存，那么缓存将重生。在生物学中，灭绝是永远的，但是在计算机科学中，有时候只是暂时离开几年而已。

由于这种无常现象的存在，这本书里我们会经常关注一些过时的概念，他们其实只是一些在当前看来不是最优的思路而已。然而，技术的变革会将一些过时的概念重新带入人们的视野。所以，理解一个概念为何过时，以及何种条件下他会重新流行起来，是非常重要的。

为了说明问题，我们来看个小例子。早期的计算机有硬件电路指令集。指令直接由硬件执行而且无法更改。然后微编程出现了（首次大规模的引入是在IBM360上），在这个系统里面，底层的解释器将硬件指令以软件形式执行。硬件电路执行被抛弃了，因为他们的灵活性不够。后来，RISC计算机被发明了，微编程开始被抛弃，因为直接硬件执行指令更加高效。现在我们又经历了解释执行的重生。Java小程序通过因特网传输到目的端再执行。执行速度并不总是那么引入关注，因为这种场景下网络延时已经是主导。就这样，钟摆在直接执行和解释执行之间已经摆动了好几个来回了，可以预见将来仍然会继续摆动。

####大内存
让我们先来回顾一下历史上硬件的发展及其对软件产生的影响。最早的大型机内存非常有限。一个全配置的IBM7900或7094，在1959年到1964年之间拥有霸主地位，但是却只有128KB的内存。对它编程都是用汇编语言，同时它运行的操作系统也是用汇编语言编程的，这都是为了节省内存。

随着时间的飞逝，FORTRAN和COBOL语言的编译进化到足够好，以至于大家都把汇编语言判了死刑。但是，当第一代商用迷你计算机（PDP-1）诞生时，它只有4096个18bit字长的内存，于是汇编语言强势回归。最终，迷你计算机获得了越来越多的内存，高级编程语言又重新流行起来。

当1980年代微型计算机初露锋芒的时候，他们只有4KB内存，汇编语言再次僵尸复活。嵌入式系统通常使用和微型计算机一样的CPU芯片（8080，Z80，后来的8086），所以他们一开始同样使用汇编。如今，他们的后代，个人电脑，拥有大量内存，他们使用C语言，C++，Java，或者其他高级语言编程。智能卡也经历了同样的历程，在达到一定的规模后，智能卡通常会有Java虚拟机来解释执行Java程序，而不是把Java程序编译成智能卡的机器语言。

####保护设备
早期的大型主机，比如IBM7090/7094，没有硬件保护，所以同一时刻只能跑一个程序。一个有bug的程序甚至可能破坏操作系统同时轻易的让系统崩溃。到了IBM360，已经有了简单的保护硬件。这种机器可以同时在内存中保存多个程序，让他们轮流执行（多程序系统）。单程序系统被宣布过时了。

至少在第一代迷你计算机出现之前是这样的——他们没有硬件保护，多程序不安全。虽然PDP-1和PDP-8没有硬件保护，最终PDP-11有了，这一特性导致了多程序系统的出现，最终使得UNIX得以诞生。

当第一台微型计算机出现的时候，他们使用的是英特尔8080CPU芯片，没有硬件保护，所以我们又退回到单程序系统——任意时刻内存中只有一个程序。直到intel80286芯片加入了硬件保护，多程序才成为可能。时至今日仍然有很多嵌入式系统没有使用硬件保护，而且只跑单程序。

现在让我们看看操作系统。第一代大型主机没有硬件保护，不能支持多程序，所以他们只能运行简单的操作系统，一次处理一个手动加载的程序。稍后它具备了硬件保护，拥有了可以处理多程序的操作系统，最后获得了所有的分时复用能力。

迷你计算机最初出现的时候，他们也没有保护硬件，一次只能运行一个手动加载的程序，虽然当时多程序系统在大型主机上已经开始使用了。渐渐地，迷你计算机获得了保护硬件，于是可以同时运行两个以上的程序。首批微型计算机一开始只能跑单程序，但是后来也支持了多程序。手持计算机和智能卡也经历了同样的历程。

在上面所列举的情形下，软件开发都受制于技术发展。拿第一代的微型计算机来说，只有4KB内存，没有保护硬件。高级语言和多程序是这样一个小型的系统无法承担的。当微型计算机进化到现代个人计算机时，他们获得了必要的软硬件设施可以支持新的特性了。这样的发展趋势必将延续多年。其他领域也存在着这种轮回，但是计算机领域这样的循环更加迅速。

####磁盘
早期的大型机大部分是基于磁带的。他们会从磁带读入程序，编译，运行，再将结果写到另一个磁带。那时候没有磁盘和文件系统的概念。当IBM在1956年引入了第一代的硬盘——RAMAC（RAndoM ACcess），事情就开始其变化了。当时RAMAC占地4平方米，可以存储5M的7bit字符，可以放得下一张中等分辨率的照片。但是年租金却高达35000美刀，如果用它的集合来存放电影，那么将是天价。最终价格降下来了，原始的文件系统出现了。

典型的代表是CDC6600,1964年诞生以后在世界上称霸最快计算机多年。用户可以创建所谓的“永久文件”，只需给他们起名字，同时寄希望于他人不使用同样的名字，比如“数据”就是个合适的文件名。这个文件系统是个一级的目录。最后，大型机发展出了复杂的层级结构文件系统，大概在MULTICS文件系统达到了高峰。

当迷你计算机开始被使用后，他们最终也拥有了硬盘。1970年出现的PDP-11的标准硬盘是RK05，容量为2.5MB，是IBM的RAMAC的一半左右，但直径只有40cm，5cm高。他一开始同样只有个一级目录。当微型计算机出现后，CP/M是最初的主流操作系统，同样只在软盘上支持一个一级目录。

####虚拟内存
虚拟内存（第三章重点讨论）通过在RAM和磁盘之间快速的来回搬运小块数据，赋予了计算机可以运行大于其系统内存的程序的能力。他经历了同样的发展过程，首先出现在大型机上，后来逐渐在迷你机和微机上得到应用。虚拟内存同样使程序动态链接库成为可能，从而可以替代一些静态编译进程序的库。MULTICS是首先支持动态链接库的系统。最终，这个思路被推广到所有系统，并在大多数UNIX和Windows系统中得到广泛应用。

在所有这些技术发展中，我们看到了某项技术首先在某个场景下被发展出来，当场景发生变化他们又被抛弃了（汇编语言，单程序系统，一级目录文件系统，等等），但在十年后的另一个场景中他们又出现了。因为这个原因，在这本书中，我们会时常研究一下那些在今天以G为存储单位的PC上看起来过时的方法和算法，他们很有可能会再次在嵌入式系统和智能卡系统中出现。
