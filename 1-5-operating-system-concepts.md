...
另一个UNIX下的重要概念是挂载文件系统。大部分的桌面电脑有一个或多个光驱，可以插入CD-ROM，DVD和蓝光光盘。他们几乎都有USB接口，可以插入U盘（或固态硬盘），还有些计算机有软盘或者外接硬盘。为了提供一个优雅的方式来处理这些可移动的存储设备，UNIX系统允许这些设备上的文件系统被附加到主文件系统树上。考虑一下图1-15的情况，在mount系统调用之前，在硬盘上的根文件系统和CD-ROM上的第二个文件系统是隔离的，他们之间没有联系。
